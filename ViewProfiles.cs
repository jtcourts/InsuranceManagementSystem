...
namespace MyLoginForm
{
    public partial class ViewClients : Form
    {
        ...
        public ViewClients(clsUser user)
        {
            objUser = user;
            InitializeComponent();
            user.getProfileInformation();
        }
        public ViewClients(clsUser user, HomePage h)
        {
            objUser = user;
            hPage = h;
            InitializeComponent();
        ...
        private void ProfileCarousel_Load(object sender, EventArgs e)
        {
            users = new List<clsUser>(); //one-to-many association
            objDBManager = new clsDBManager(connectionString);

            users = objDBManager.LoadClientsFromDb();
